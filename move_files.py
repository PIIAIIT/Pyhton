...
DEST_DIRS = ['Music','Movies','Pictures','Documents','Applications','Other', 'OSU']
...
for f_ext, f_list in files_mapping.items():
    ...
    elif f_ext in EXT_COMPR or f_ext in EXT_DOCS:
        for file in f_list:
    ...
    elif f_ext in EXT_OSU:
        for file in f_list:
            os.rename(os.path.join(DOWNLOADS_PATH, file), os.path.join(BASE_PATH, 'OSU', file))
